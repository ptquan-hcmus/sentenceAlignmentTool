...
def pre_translate(EL, VN):
   ...
   if EL_len == 0 or VN_len == 0:
      return []
   data = [[EL[i], []] for i in range(EL_len)]
   ...
   for i in range(int(EL_len/2)):
      ...
      if max_indexes_top != []:
         for index in max_indexes_top:
            if check_list[index] == -1 and abs(index - top + 1) <= 10:
               check_list[index] = i
...
app.config["TEXT_UPLOAD"] = os.path.dirname(os.path.realpath(__file__)) + "/static/cache"
...
def hello():
   ...
   return render_template('index.html', data = [], check_box = [])
